class ActivitiesController < ApplicationController
	...
	def create
		...
		response.xpath("//iati-activity").each do |activity|
			...
			if rcpt_country and (!rcpt_country.attr("percentage") or rcpt_country.attr("percentage") == "100")
				...
				Nokogiri::XML(activity.to_s).xpath("//description").each do |desc|
					...
					if desc.attr("type") and (!desc.attr("xml:lang") or desc.attr("xml:lang") == "en")
